...
//    PX4_INFO("gain att Roll %f, Pitch %f, Yaw %f", (double) attitude_gain(0), (double) attitude_gain(1),
//             (double) attitude_gain(2));
...
//    PX4_INFO("Rates Sp No Lim Roll:%f, Pitch: %f, Yaw: %f",
//             (double)_rates_sp.rpy(0), (double)_rates_sp.rpy(1),
//             (double)_rates_sp.rpy(2));
...
//    PX4_INFO("Rates Error Roll:%f, Pitch: %f, Yaw: %f",
...
//    _ctrl_status.rates_prev_filtered = rates_filtered;
    ...
    /* Update Integral if nonzero setpoint and not saturated */
    for (int i = AXIS_INDEX_ROLL; i < AXIS_COUNT; i++) {
        ...
        if (((i == AXIS_INDEX_ROLL && (fabs(_compensated_V_sp(i)) > 0.25f)) || // TODO: make this a param
             ((i > AXIS_INDEX_ROLL) && (fabs(_compensated_V_sp(i)) > 0.25f))) &&
                _compensated_V_sp(i) > -1.0f && _compensated_V_sp(i) < 1.0f) {
...
//    PX4_INFO("Integral Status roll: %f, pitch: %f, yaw: %f",
//             (double)_ctrl_status.rates_int(0),
//             (double)_ctrl_status.rates_int(1),
//             (double)_ctrl_status.rates_int(2));
...
AttitudeController::compensateThrusterDynamics() {
    ...
    const float F_sigmoid_c1 = 10.0f; //TODO: move to param?
    const float F_sigmoid_c2 = 34.138f;
    const float Tjoin = .15f;

    /* Convert RPY Torque Sp to Voltage SP */
    ...
    //TODO: Get VTorq coefficient and use here instead.
    //Scale Down roll since gains resolution can't be set lower on GCS
    roll_V_   = AController::poly_abs_discontinuos(_constants.VTorq_coefficients, roll_T_) / 100.0f;

    /* Pitch and Yaw curves are discontinuous, apply sigmoid to smooth near zero terms */
    if (fabs(pitch_F_) <= Tjoin){
        pitch_V_ = F_sigmoid_c2 * pitch_F_ / ( 1 + F_sigmoid_c1 * fabs(pitch_F_));
    }
    else{
        pitch_V_  = AController::poly_abs_discontinuos(_constants.VT_coefficients, pitch_F_);
    }

    if (fabs(yaw_F_) <= Tjoin){
        yaw_V_ = F_sigmoid_c2 * yaw_F_ / ( 1 + F_sigmoid_c1 * fabs(yaw_F_));
    }
    else{
        yaw_V_  = AController::poly_abs_discontinuos(_constants.VT_coefficients, yaw_F_);
    }
...
//    PX4_INFO("Vnom %f: ", (double) _state.power.bat_V_nom);
...
//    _compensated_V_sp(2) = yaw_V_       / _state.power.bat_V_nom;
    _compensated_V_sp(0) = roll_V_      / 8.4f;
    _compensated_V_sp(1) = -pitch_V_     / 8.4f;
    _compensated_V_sp(2) = -yaw_V_       / 8.4f;
    _compensated_V_sp(3) = thrust_V_;
//    _compensated_V_sp(0) = 0.0f;
//    _compensated_V_sp(1) = 0.0f;
//    _compensated_V_sp(2) = 0.0f;
//    _compensated_V_sp(3) = 0.0f;

//    PX4_INFO("Compensated V Roll:%f, Pitch: %f, Yaw: %f, Thrust: %f",
//             (double)_compensated_V_sp(0), (double)_compensated_V_sp(1),
//             (double)_compensated_V_sp(2), (double)_compensated_V_sp(3));
